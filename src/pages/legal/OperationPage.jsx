// src/pages/legal/OperationPage.jsx
/* eslint-disable */
...
export default function OperationPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await getLegalDoc("operation");
        if (!alive) return;
        if (data && (data.content || "").trim()) setDoc(data);
        else setDoc(null);
      } catch (e) {
        console.warn("[OperationPage] load failed:", e?.message || e);
  ...
  return (
    <Page>
      <Sheet>
        ...
        <Head>
          <Title>{LEGAL_DEFAULTS.operation.title}</Title>
        </Head>

        <Divider />

        <Body>{LEGAL_DEFAULTS.operation.content}</Body>
      </Sheet>
    </Page>
  );
}

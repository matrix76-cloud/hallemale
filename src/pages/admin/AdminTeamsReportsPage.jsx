...
export default function AdminTeamsReportsPage() {
  ...
  const [blockTarget, setBlockTarget] = useState(null); // { reportId, clubId, name }
  ...
  const load = async (sf = statusFilter) => {
    ...
    setErr("");
    try {
      const list = await listTeamReports({ statusFilter: sf });
      setRows(Array.isArray(list) ? list : []);
      setPage(1);
    } catch (e) {
      console.error("[AdminTeamsReportsPage] load failed", e);
      setRows([]);
  ...
  const openBlockFromReport = (report) => {
    setBlockTarget({
      ...
      name: report.clubName || report.clubId,
    });
  ...
  const handleConfirmBlock = async () => {
    ...
    setBlockBusy(true);
    try {
      await blockTeam({ clubId: t.clubId, reason, byAdmin: "admin" });
      // 신고 자체도 resolved 처리
      if (t.reportId) {
      ...
      <HeaderRow>
        <div>
          ...
          <Sub style={{ marginTop: 4 }}>
            사용자가 신고한 팀 목록을 검토하고 차단 또는 기각할 수 있습니다.
          </Sub>
        </div>
            ...
            <Table>
              ...
              {viewRows.map((r) => (
                <Row key={r.id}>
                  <Cell>
                    <NameStack>
                      <Trunc>{r.clubName || r.clubId || "-"}</Trunc>
                      <Mono>
                        <Trunc>{r.clubId}</Trunc>
                      </Mono>
                    </NameStack>
                  </Cell>
      ...
      {blockOpen && (
        ...
        >
          <Modal onClick={(e) => e.stopPropagation()}>
            ...
            <ModalLabel>
              대상: <strong>{blockTarget?.name}</strong> ({blockTarget?.clubId})
            </ModalLabel>

...
function leftPad11(d = "") {
  return safeStr(d).padStart(11, "0");
}
...
export default function FindPasswordPage() {
  ...
  const canResetPw = useMemo(() => {
    const p1 = safeStr(newPw);
    const p2 = safeStr(newPw2);
    return phoneVerified && p1.length >= 6 && p1 === p2 && !busy;
  }, [phoneVerified, newPw, newPw2, busy]);
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      const expected = safeStr(sentOtp || devCode);
      const got = safeStr(onlyDigits(codeInput));
  ...
  const handleResetPassword = async () => {
    ...
    setBusy(true);
    try {
      const e164 = safeStr(sentToE164 || toE164KR(phone));
      if (!e164) {
        window.alert("전화번호를 다시 확인해 주세요.");
        return;
      }

      const uid = await findUidByPhoneE164(e164);
      if (!uid) {
        window.alert("해당 전화번호로 가입된 계정을 찾을 수 없습니다.");
        return;
      }

      await resetPasswordViaProxy({ uid, newPassword: safeStr(newPw), app: "halle" });
      window.alert("비밀번호가 변경되었습니다. 로그인해 주세요.");
      navigate("/login", { replace: true });
    } catch (e) {
      window.alert(`비밀번호 변경에 실패했습니다. ${String(e?.message || e)}`);
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <>
      ...
      <Wrap>
            ...
            <>
              <Field>
                ...
                <Input
                  ...
                  disabled={busy}
                />
                <Helper>비밀번호는 6자 이상을 권장합니다.</Helper>
              </Field>
              ...
              <Field>
                ...
                <Input
                  ...
                  disabled={busy}
                />
                {safeStr(newPw2) && safeStr(newPw) !== safeStr(newPw2) ? (
                  <Helper>비밀번호가 서로 일치하지 않습니다.</Helper>
                ) : null}
              </Field>
            </>
          ) : null}
        ...
        <BtnArea>
          {phoneVerified ? (
            <Button fullWidth type="button" onClick={handleResetPassword} disabled={!canResetPw}>
              {busy ? "변경 중..." : "비밀번호 변경"}
            </Button>
          ) : null}

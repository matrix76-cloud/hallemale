...
/* ── 부가가치세 ──────────────────────────────────────────────
 * 부가세는 두 갈래로 갈린다. 섞으면 안 된다.
 *
 *  ① 구장 이용료(결제액 80,000)의 부가세 → **구장주 몫**.
 *     재화·용역의 공급자는 구장 사업자이고 우리는 통신판매중개자다(상세·결제 화면에
 *     그렇게 고지하고 있다). 그 매출은 우리 장부에 잡히지 않으므로 원장에도 확정 저장하지
 *     않는다 — 구장주의 과세유형(일반/간이/면세: 학교·공공)에 따라 세액이 갈리는데,
 *     결제 시점에 그걸 단정해 박아두면 틀린 숫자가 원장에 남는다.
 *     정산 명세에서 `business.taxType` 을 보고 참고값으로만 계산해 보여준다.
 *
 *  ② 플랫폼 이용료(4,000)의 부가세 → **우리 몫**. 우리 매출이므로 신고·납부하고
 *     구장주에게 수수료 세금계산서를 발행한다(구장주는 매입세액 공제). 우리는 일반과세자라
 *     구장주 과세유형과 무관하게 항상 10%다 → 결제 시점에 확정해 원장에 남길 수 있다.
 *
 * ⚠️ 플랫폼 이용료는 **부가세 포함가**다. 별도로 보면 80,000 결제에서 4,400 을 떼야 하고
 *    정산액이 75,600 이 되는데, 현재 정산은 4,000 만 떼고 76,000 을 준다
 *    (venueAmount = amount - platformFee). 구현이 곧 정책이므로 포함가로 못 박는다.
 *    5% 를 "부가세 별도"로 바꾸려면 요율이 아니라 이 전제부터 바꿔야 한다.
 *
 * ⚠️ functions/payments/toss.js 의 VAT_RATE·splitVat 과 같은 값·같은 식을 유지할 것.
 */
export const VAT_RATE = 0.1;

/**
 * 부가세 포함가 → { supply(공급가액), vat(부가세) }.
 * 부가세를 반올림하고 공급가액은 뺄셈으로 구한다 — 그래야 supply + vat 이 원가와
 * 1원도 안 어긋난다(정산 검증이 이 항등식을 쓴다. platformFee 쪽 규약과 같은 이유).
 */
export function splitVat(taxIncluded) {

...
- **[중대] 숨김 처리한 글이 상세 URL 로는 그대로 열린다** — 사용자측 목록은 `data.hidden` 을 거르지만
  (`communityService.js:107`), 상세 로더(`loadCommunityPostDetail:185-193`)는 **내 차단/숨김 목록만**
  확인하고 어드민 숨김은 보지 않는다. 알림·공유 링크·뒤로가기로 들어오면 신고된 글이 계속 보인다.
  숨김 조치의 실효가 절반이다.
...
- **[중대] 10-13 과 같은 숨김 우회**를 공유한다(같은 뿌리).
...
- **[중대] 잠금 사유 입력을 "취소" 해도 방이 잠긴다** — `:315-319`
  ```js
  reason = window.prompt(...) || "";
  if (reason === null) return;   // || "" 때문에 절대 참이 안 된다
  ```
  취소를 누른 운영자는 잠그지 않았다고 믿는데 방은 잠겨 있다.
- **[중대] 메시지를 지워도 목록의 "최신 메시지" 에 그대로 남는다** — `deleteChatMessageByAdmin`
  (`adminChatService.js:222-228`)이 방 문서의 `lastMessageText` 를 갱신하지 않는다. 어드민 채팅
  목록(`AdminChatListPage.jsx:429`)과 **사용자 채팅 목록**(`ChatListPage.jsx:402`) 미리보기에
  지운 문장이 계속 보인다. 부적절 메시지 삭제의 실효가 없다.

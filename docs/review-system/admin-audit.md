...
## 4차 라운드 (2026-08-06) — 잔여 화면 레이어 5개

서비스 계층만 보고 넘겼던 10-13·10-14·10-15·10-21·10-23 의 **화면 로직**을 봤다. 기록 전용.

**10-13 커뮤니티 글** (`AdminCommunityPostsPage.jsx`)

- **[중대·수정 완료] 숨김 처리한 글이 상세 URL 로는 그대로 열린다** — 사용자측 목록은 `data.hidden` 을
  거르지만(`communityService.js:107`), 상세 로더(`loadCommunityPostDetail:185-193`)는 **내 차단/숨김
  목록만** 확인하고 어드민 숨김은 보지 않았다. 알림·공유 링크·뒤로가기로 들어오면 신고된 글이 계속
  보였다 — 숨김 조치의 실효가 절반이었다.
  → 상세 로더에 `data.hidden` 차단을 추가하고(`reason: "hidden_by_admin"`),
  화면은 "존재하지 않습니다" 대신 **"운영자가 숨긴 게시글입니다"** 로 사유를 알린다
  (`CommunityDetailPage.jsx:757-763`). 어드민 상세(10-14)는 별도 로더라 계속 열람 가능하다.
- **[중대] "공지로 고정" 이 최근 30건 밖의 글에는 안 먹는다** — 사용자 목록은 `createdAt desc` 로
  30건을 받은 **뒤에** 그 안에서 pinned 를 위로 올린다(`communityService.js:93,158-161`).
  오래된 글을 공지로 고정하면 첫 페이지에 안 올라온다. 어드민 화면에는 "공지"로 표시된다.
- **[경미] 검색이 제목·작성자만** — 본문은 안 걸린다. 신고 내용으로 글을 찾을 수 없다.
- **[경미] 삭제 확인 문구가 지키지 못할 약속** — "댓글/좋아요까지 함께 사라지고" 라고 하지만
  댓글 500건이 넘으면 실제로는 남는다(그룹 ③ 기록 참고).

**10-14 커뮤니티 글 상세** (`AdminCommunityPostDetailPage.jsx`)

- **[중대·수정 완료] 10-13 과 같은 숨김 우회**를 공유했다(같은 뿌리, 위 수정으로 함께 해소).
- **[경미] 누가 언제 숨겼는지 화면에 안 나온다** — 서비스는 `hiddenBy`·`hiddenAt` 을 실어 보내는데
  (`adminCommunityService.js:150-152`) 화면은 "숨김" 알약만 찍는다. 조치 이력을 볼 수 없다.
- **[경미] 신고 맥락이 끊긴다** — 10-15 에서 넘어와도 신고 사유가 상세에 안 보여, 무엇 때문에
  들어왔는지 모른 채 판단하게 된다.
- **[경미] 댓글 전량 로드**(`orderBy createdAt asc`, `limit` 없음).
- 이미지 필드(`media.images`)는 사용자 저장 경로와 일치한다 — 결함 아님.

**10-15 커뮤니티 신고** (`AdminCommunityReportsPage.jsx`)

- **[중대] 글을 삭제·숨겨도 같은 글의 다른 신고는 대기열에 남는다** — `:325-345` 는 누른 신고
  1건만 `resolved` 로 바꾼다. 남은 신고를 누르면 이미 지워진 글로 이동해 "게시글을 찾을 수 없습니다"
  가 뜬다. 10-05·10-09 와 완전히 같은 모양이다.
- **[경미] 처리자가 항상 `"admin"`**(`:326`, `:344`, `:359`).
- **[경미] `community_reports` 전수 조회**(`postReportService.js:66-76`).

**10-21 지난 경기** (`AdminGamesPastPage.jsx`)

- **[중대] 지역 필터를 바꿔도 목록이 그대로다** — 지역·키워드는 **조회 시점에 서비스 안에서**
  적용되는데(`adminGamesService.js:248-250`), 화면은 마운트 1회와 검색창 Enter 에서만 `load()` 를
  부른다(`:241-243`, `:273-275`). 드롭다운을 바꾸면 `page` 만 1로 돌아가고 결과는 그대로다.
  옆에 조회 버튼도 없어 "필터가 고장났다" 로 보인다.
- **[중대] "스코어 검색" 이라고 적어놓고 스코어로는 안 걸린다** — 검색 대상은 팀명·장소·경기규격
  뿐이다(`matchKeyword:166-171`). 플레이스홀더가 없는 기능을 광고한다.
- **[중대] "기간" 열이 경기 날짜가 아니라 문서 수정 시각** — `when: fmtRelative(updatedAt || scheduledAt)`
  (`:243`). 결과를 나중에 입력하면 실제 경기일과 다른 값이 뜬다. "지난 경기" 목록에서 가장 중요한
  열이 어긋난다.
- **정정(2차 기록 수정)**: 지난 경기 쿼리에는 `orderBy("updatedAt","desc")` 가 **있다**. 2차에서
  "예정 경기와 동일하게 정렬 없는 200건" 이라고 적은 것은 틀렸다. 다만 `updatedAt` 이 없는 문서가
  결과에서 통째로 빠지는 문제는 그대로다.

**10-23 채팅방 상세** (`AdminChatRoomDetailPage.jsx`)

- **[중대·수정 완료] 잠금 사유 입력을 "취소" 해도 방이 잠긴다** — `:315-319`
  ```js
  reason = window.prompt(...) || "";
  if (reason === null) return;   // || "" 때문에 절대 참이 안 된다
  ```
  취소를 누른 운영자는 잠그지 않았다고 믿는데 방은 잠겨 있었다.
  → `|| ""` 를 씌우기 전에 `null` 판정을 먼저 하도록 바꿨다.
- **[중대·수정 완료] 메시지를 지워도 목록의 "최신 메시지" 에 그대로 남는다** — `deleteChatMessageByAdmin`
  (`adminChatService.js:222-228`)이 방 문서의 `lastMessageText` 를 갱신하지 않았다. 어드민 채팅
  목록(`AdminChatListPage.jsx:429`)과 **사용자 채팅 목록**(`ChatListPage.jsx:402`) 미리보기에
  지운 문장이 계속 보였다 — 부적절 메시지 삭제의 실효가 없었다.
  → 삭제 직후 남아 있는 마지막 메시지로 `lastMessageText`/`lastMessageAt`/`lastMessageFromUid` 를
  되맞춘다(`refreshLastMessageMeta`). 미리보기 문구는 보낼 때와 같은 형식("사진 N장 · 본문")을 쓰고,
  남은 메시지가 없으면 비운다. 갱신 실패가 삭제를 되돌리지는 않는다.
- **[중대] 이미지 메시지를 지워도 파일은 Storage 에 남는다** — 문서만 지운다. URL 을 아는 사람은
  계속 볼 수 있다. 방 삭제(`:233-254`)도 마찬가지.
- **[경미] 메시지 전량 로드** — `limit` 없이 `createdAt asc` 전부.
- **[COMMON] 잠금은 클라이언트에서만 강제된다** — 송신 함수가 막을 뿐(`chatService.js:414`, `:563`),
  규칙은 `chatRooms/{id}/{sub=**}` 쓰기를 `signedIn()` 으로 열어 둔다(`firestore.rules:196-197`).
  같은 줄의 `allow read: if true` 때문에 **로그인 없이도 모든 채팅 내용을 읽을 수 있다** —
  커뮤니티·채팅 감사에서 올라온 항목과 같은 뿌리다.

---

### 미착수 (다음 회차)

관리자 32화면 **전부 1차 감사 완료**(2026-08-06). 다음 회차 대상은 자기 구현이라 제외했던
10-12 분쟁/신고 · 10-17 푸시 발송 · 10-31 발송 로그 · 10-32 앱 업데이트 4개다.

...
할래말래(Hallamaella) 웹의 **개발 전용 화면 리뷰 허브**입니다. 기획자·개발자·리뷰어(AI, Claude Code)가 **같은 화면을 나란히 보며 피드백을 남기고 답하는** 도구입니다. 이 문서 하나로 다른 컴퓨터에서도 리뷰 시스템을 이해하고 그대로 운영할 수 있게 정리했습니다.
...
- **구성(화면당 3분할)**
  ...
  - 우측 하단: **기록 스레드**(개발자 ↔ AI, Firestore 실시간 공유)
...
### 기록 남기기 (우측 하단 스레드)
- 작성자를 **개발자 / AI** 중 선택 → 텍스트 입력 → `Enter` 전송 (`Shift+Enter` 줄바꿈).
- 스샷 첨부: 파일 선택 또는 클립보드 `Ctrl+V` 붙여넣기. (Firestore 1MB 제한 대비 자동 리사이즈+JPEG 압축)
- **답글 구조**: 개발자 글(root) 밑에 AI 답글(`replyTo`)이 들여쓰기로 붙습니다.

### 화면에 핀 찍기 (좌측, 모바일 화면만)
- `핀 찍기` 켠 뒤 화면을 클릭하면 그 지점에 `작성자+번호`(예: `AI1`) 라벨 핀이 박힙니다.
...
### 미답변 뱃지 (코랄색 숫자)
- **개발자가 올린 질문/지시(root) 중 AI 답글이 안 달린 것**의 수를 도메인 탭·화면 칩에 코랄 뱃지로 표시합니다.
- AI(리뷰어)는 이 뱃지를 보고 **미답변부터** 처리하면 됩니다.

---

## 4. 데이터 / 백엔드

- **컬렉션**: Firestore `reviewThreads` (배포 URL에서도 개발자·AI가 같은 기록을 실시간 공유)
- **문서 1건 = 기록 1건**:
  ```
...
## 6. AI(Claude Code)와 협업하는 지시서 포맷

여러 작업을 병렬로 돌리거나, 리뷰에서 나온 수정 요청을 AI에게 넘길 때 쓰는 표준 지시서 포맷입니다. **파일 소유권을 명확히 나눠 충돌을 방지**하는 게 핵심입니다.
...
`docs/review-system/memory/` 에는 이 프로젝트를 AI와 작업하며 쌓아온 **컨텍스트 메모**를 넣어뒀습니다. 다른 컴퓨터에서 리뷰/개발을 이어갈 때 배경 지식으로 참고하세요.

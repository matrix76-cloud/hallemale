...
//
// ⚠️ 50005 "결제요청(구장 승인 완료)" = 카카오 반려(2026-08-07). 결제 독촉성으로 판정됐다.
//    구장 승인 직후 2시간 결제창이 열렸다는 사실을 알릴 알림톡이 없다 → 앱 푸시만 나간다.
//    재접수하려면 "결제하면 확정 / 안 하면 자동취소" 같은 유도·경고 문구를 빼고
//    승인 사실과 마감 시각만 고지하는 형태로 다시 써야 한다.
const TEMPLATES = {
  matchConfirmedDirect: { id: "50001", name: "경기확정(직접입력)" }, // ✅ 승인 2026-07-25
  matchCanceledDirect: { id: "50002", name: "경기취소(직접입력)" }, // ✅ 승인 2026-07-25
  matchConfirmed: { id: "50003", name: "경기확정(제휴구장)" }, // ✅ 승인 2026-08-07
  matchCanceled: { id: "50004", name: "경기취소·환불(제휴구장)" }, // ✅ 승인 2026-08-07
  paymentReminder: { id: "50006", name: "결제마감임박" }, // ✅ 승인 2026-08-07
  soloConfirmed: { id: "50007", name: "예약확정(개인예약)" }, // ✅ 승인 2026-08-07
  soloCanceled: { id: "50008", name: "예약취소·환불(개인예약)" }, // ✅ 승인 2026-08-07
};
...
※ 원활한 예약 진행을 위해 예약자 연락처가 해당 구장에 전달됩니다.`,
  ...
  // 환불액 0원(당일 취소·이용 시작 후)도 같은 템플릿으로 나간다.
  //   "환불이 진행됩니다"로 단정하면 0원 건에서 거짓 안내가 되므로 본문은 취소 사실만 알리고,
  //   전액/부분/불가는 #{환불기준} 한 줄이 설명한다(refundPolicy.refundDecision 의 판정을 그대로 문장화).
  matchCanceled: (v) =>
`[할래말래] 경기 취소 안내
...
- 취소사유 : ${v.사유}
- 결제금액 : ${v.금액}원
- 환불금액 : ${v.환불금액}원 (${v.환불기준})

환불금액은 결제하신 수단으로 자동 처리되며,
카드사 사정에 따라 영업일 기준 3~5일 정도 소요될 수 있습니다.

자세한 내용은 앱에서 확인하실 수 있습니다.`,

  // 마감 30분 전, 아직 안 낸 쪽에게만. 매칭이면 그 팀 부담분, 개인 예약이면 예약 전액.
  paymentReminder: (v) =>
`[할래말래] 결제 마감 임박 안내
...
마감 시각이 지나면 예약이 자동 취소됩니다.
이미 결제된 금액이 있으면 함께 환불됩니다.`,

  // 개인 예약(매칭 아님) — 상대팀·인원 변수가 없고, 대신 구장안내(ownerNote)가 들어간다.
  soloConfirmed: (v) =>
`[할래말래] 예약 확정 안내
...
이용 시작 전까지 앱에서 취소할 수 있습니다.

※ 원활한 예약 진행을 위해 예약자 연락처가 해당 구장에 전달됩니다.`,

  // 환불기준 판정은 제휴구장(matchCanceled)과 같은 refundPolicy.refundDecision 을 쓴다.
  soloCanceled: (v) =>
`[할래말래] 예약 취소 안내
